...
class Config:
    ...
    try:
        RAPIDAPI_KEY = (
            st.secrets.get("RAPIDAPI_KEY", "demo_key")
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        try:
            ...
            # Créer des index
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_matches_odds ON matches(home_odds, draw_odds, away_odds, over_25_odds, under_25_odds)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_matches_date ON matches(match_date)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_matches_today ON matches(is_today_match)"
            )
            cursor.execute(
    ...
    def get_matches_with_complete_odds(
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(DISTINCT league_name) FROM matches WHERE league_name IS NOT NULL"
            )
    ...
    def _parse_api_matches(self, events: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for event in events:
            try:
                ...
                league = (event.get("league_name") or "").lower()
                if Config.ALLOWED_LEAGUES and league not in Config.ALLOWED_LEAGUES:
                    continue
...
class OddsSimilarityEngine:
    ...
    def calculate_similarity_cosine(
    ...
    def find_similar_matches(
        ...
        for idx, row in historical_df.iterrows():
            try:
                ...
                if np.any(np.isnan(historical_vector)) or np.any(
                    historical_vector <= 0
                ):
                    continue

                similarity_score = self.calculate_similarity_cosine(
                    target_vector, historical_vector
                )
    ...
    def predict_match_outcome(self, match_odds: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            ...
            matches_with_results = [
                m for m in similar_matches if m["match_data"].get("result") is not None
            ]
...
def get_prediction_recommendation(
    home_pred, draw_pred, away_pred, over_pred, confidence
):
...
def main():
    ...
    st.markdown(
        "**Intelligence artificielle pour les paris sportifs basée sur l'analyse de similarité des cotes**"
    )
    ...
    with st.sidebar.expander("⚙️ Paramètres d'Analyse", expanded=False):
        ...
        confidence_threshold = st.slider(
            "Confiance minimum pour recommandation", 40, 90, 60
        )
    ...
    with col1:
        if st.button(
            "🔄 Actualiser les matchs du jour", type="primary", use_container_width=True
        ):
                ...
                if today_matches:
                    ...
                    with st.spinner("🔮 Génération des prédictions..."):
                        ...
                        for match in today_matches:
                            ...
                            prediction = similarity_engine.predict_match_outcome(
                                match_odds
                            )
                            prediction["event_id"] = match["event_id"]
                            prediction["prediction_date"] = datetime.now().strftime(
                                "%Y-%m-%d"
                            )
    ...
    if today_matches_df.empty:
        st.info(
            "📅 Aucun match programmé pour aujourd'hui. Cliquez sur 'Actualiser' pour récupérer les derniers matchs."
        )
        ...
        with col1:
            leagues = ["Toutes"] + sorted(
                today_matches_df["league_name"].unique().tolist()
            )
        ...
        with col2:
            ...
            selected_confidence = st.selectbox(
                "⭐ Niveau de confiance", confidence_levels
            )
        ...
        if selected_league != "Toutes":
            filtered_df = filtered_df[
                filtered_df["league_name"].str.lower() == selected_league.lower()
            ]
        ...
        if selected_confidence != "Toutes":
            if selected_confidence == "Élevée (≥80%)":
                ...
                filtered_df = filtered_df[
                    (filtered_df["confidence_score"] >= 60)
        ...
        for idx, match in filtered_df.iterrows():
            ...
            try:
                match_time = (
                    match["start_time"].split("T")[1][:5]
                    if "T" in str(match["start_time"])
                    else "TBD"
                )
            except:
                match_time = "TBD"
            ...
            with col3:
                if match.get("confidence_score") is not None:
                    ...
                    st.metric(
                        "🔍 Matchs similaires",
                        int(match.get("similar_matches_count", 0)),
                    )
                    st.metric(
                        "📈 Similarité moy.", f"{match.get('avg_similarity', 0):.3f}"
                    )
        ...
        if not filtered_df.empty:
            ...
            with col2:
                high_confidence = len(
                    filtered_df[filtered_df["confidence_score"] >= 80]
                )
    ...
    st.markdown("---")
    st.markdown(
        "**🔮 Prédictions Automatiques Pinnacle** - IA basée sur l'analyse de similarité des cotes historiques"
    )
    st.caption(
        "⚠️ Les prédictions sont fournies à titre informatif uniquement. Pariez de manière responsable."
    )
