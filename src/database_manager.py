...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            pinnacle_logger.log_database_operation(
                "init_database", "all_tables", None, execution_time
            )

            self.logger.info(
                f"Database initialized successfully in {execution_time:.3f}s"
            )
    ...
    def save_match(self, match_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                pinnacle_logger.log_database_operation(
                    "save_match", "matches", affected_rows, execution_time
                )
            ...
            self.logger.error(
                f"Failed to save match {match_data.get('event_id', 'unknown')}: {e}"
            )
            return False
    ...
    def _calculate_data_quality_score(self, match_data: Dict[str, Any]) -> float:
        ...
        if all(
            match_data.get(field) for field in ["home_odds", "draw_odds", "away_odds"]
        ):
            total_prob = sum(
                1 / match_data[field]
    ...
    def _update_league_stats(
        self, cursor: sqlite3.Cursor, league_id: int, conn: sqlite3.Connection
    ):
    ...
    def get_matches_with_complete_odds(
        self, limit: Optional[int] = None, min_quality_score: float = 0.5
    ) -> pd.DataFrame:
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                pinnacle_logger.log_database_operation(
                    "get_matches_with_complete_odds", "matches", len(df), execution_time
                )
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    "SELECT COUNT(DISTINCT league_id) FROM matches WHERE league_id IS NOT NULL"
                )
                stats["total_leagues"] = cursor.fetchone()[0]

                # Plage de dates
                cursor.execute(
                    "SELECT MIN(start_time), MAX(start_time) FROM matches WHERE start_time IS NOT NULL"
                )
                ...
                cursor.execute(
                    "SELECT AVG(data_quality_score) FROM matches WHERE data_quality_score IS NOT NULL"
                )
                avg_quality = cursor.fetchone()[0]
                stats["avg_data_quality"] = (
                    round(avg_quality, 3) if avg_quality else None
                )
                ...
                stats["top_leagues"] = [
                    {"name": row[0], "matches": row[1]} for row in cursor.fetchall()
                ]
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                pinnacle_logger.log_database_operation(
                    "get_database_stats", "multiple", None, execution_time
                )
    ...
    def optimize_database(self) -> Dict[str, Any]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                self.logger.info(
                    f"Database optimization completed in {execution_time:.2f}s"
                )
