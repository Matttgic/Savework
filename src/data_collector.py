...
class PinnacleDataCollector:
    ...
    def get_sports(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/kit/v1/sports", headers=self.headers, timeout=30
            )
    ...
    def get_markets(
        ...
        params = {"sport_id": sport_id, "is_have_odds": is_have_odds}
    ...
    def get_archive_events(
        self, sport_id: Optional[int] = None, days_back: Optional[int] = None
    ) -> Optional[Dict[str, Any]]:
    ...
    def extract_odds_from_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not main_period:
                self.logger.debug(
                    f"No main period found for event {event_data.get('event_id')}"
                )
                return match_data
            ...
            self.logger.error(
                f"Error extracting odds from event {event_data.get('event_id', 'unknown')}: {e}"
            )
            return {}
    ...
    def collect_current_markets(self) -> Dict[str, int]:
        ...
        for event in events:
            try:
                ...
                if match_data and self._has_complete_odds(match_data):
                    ...
                    if collected <= 5:
                        self.logger.debug(
                            f"Saved match: {match_data.get('home_team')} vs {match_data.get('away_team')}"
                        )
                ...
                self.stats["failed_requests"] += 1
                self.logger.error(
                    f"Error processing event {event.get('event_id', 'unknown')}: {e}"
                )
                continue
        ...
        pinnacle_logger.log_data_collection(
            "current_markets", len(events), collected, errors, duration
        )

        self.logger.info(
            f"Current markets collection completed: {collected} saved, {errors} errors"
        )
    ...
    def collect_historical_data(
        ...
        with ThreadPoolExecutor(max_workers=Config.MAX_CONCURRENT_REQUESTS) as executor:
            ...
            for i in range(0, len(events), batch_size):
                ...
                self.logger.info(
                    f"Progress: {progress:.1f}% ({collected_count} saved, {errors_count} errors)"
                )
        ...
        duration = time.time() - self.stats["start_time"]
        pinnacle_logger.log_data_collection(
            f"historical_{source}", len(events), collected_count, errors_count, duration
        )
        ...
        self.logger.info(
            f"Historical collection completed: {collected_count} saved, {errors_count} errors"
        )
    ...
    def _process_historical_event(self, event: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error processing historical event {event.get('event_id', 'unknown')}: {e}"
            )
            return False
    ...
    def _has_complete_odds(self, event_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            money_line = main_period.get("money_line", {})
            if not all(
                money_line.get(field) and money_line.get(field) > 1.0
            ...
            has_25_total = any(
                float(points) == 2.5
                and total_data.get("over")
                and total_data.get("over") > 1.0
                and total_data.get("under")
                and total_data.get("under") > 1.0
                for points, total_data in totals.items()
    ...
    def get_collection_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "processing_rate": (
                (self.stats["matches_processed"] / duration) if duration > 0 else 0
            ),
        }
