...
class OddsSimilarityEngine:
    ...
    def calculate_odds_vector(self, odds_data: Any) -> np.ndarray:
        ...
        try:
            if isinstance(odds_data, pd.Series):
                ...
                for key in required:
                    if (
                        odds_data.get(key) is None
                        and odds_data.get(f"{key}_odds") is None
                    ):
                        raise KeyError(key)
            ...
            if np.any(np.isnan(vector)) or np.any(vector <= 0):
                raise SimilarityError(
                    "Invalid odds vector: contains NaN or non-positive values"
                )
    ...
    def calculate_similarity_cosine(
        ...
        try:
            ...
            v1 = np.log(vector1) - np.log(vector1).mean()
            v2 = np.log(vector2) - np.log(vector2).mean()
    ...
    def calculate_similarity_euclidean(
        self, vector1: np.ndarray, vector2: np.ndarray
    ) -> float:
    ...
    def calculate_similarity(
        ...
        self.validator.validate_similarity_parameters(
            method, 0.5, 1
        )  # Validation de base
    ...
    def find_similar_matches(
        ...
        if historical_df.empty:
            self.logger.warning(
                "No historical data available for similarity calculation"
            )
            return []

        self.logger.info(
            f"Analyzing {len(historical_df)} historical matches using {method} method"
        )
        ...
        if len(historical_df) > 1000:  # Traitement parallèle pour les gros datasets
            similarities = self._calculate_similarities_parallel(
                historical_df, target_vector, method
            )
        else:
        ...
        high_similarity_matches = [
            match for match in similarities if match["similarity"] >= threshold
        ]
        ...
        if len(high_similarity_matches) < min_matches:
            ...
            self.logger.info(
                f"Only {actual_above_threshold} matches above {threshold*100:.1f}% threshold"
            )
            self.logger.info(
                f"Returning top {len(result_matches)} most similar matches"
            )
        else:
            result_matches = high_similarity_matches
            self.logger.info(
                f"Found {len(result_matches)} matches above {threshold*100:.1f}% threshold"
            )
        ...
        pinnacle_logger.log_similarity_calculation(
            method, len(result_matches), execution_time, threshold
        )

        # Métriques détaillées
        pinnacle_logger.log_performance_metrics(
            "similarity_calculation",
            {
                ...
                "avg_similarity": (
                    np.mean([m["similarity"] for m in result_matches])
                    if result_matches
                    else 0
                ),
    ...
    def _calculate_similarities_sequential(
        self, historical_df: pd.DataFrame, target_vector: np.ndarray, method: str
    ) -> List[Dict[str, Any]]:
        ...
        for idx, row in historical_df.iterrows():
            try:
                ...
                self.logger.debug(
                    f"Error calculating similarity for event {row.get('event_id', 'unknown')}: {e}"
                )
                continue
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for chunk in chunks:
                future = executor.submit(
                    self._process_chunk, chunk, target_vector, method
                )
    ...
    def _process_chunk(
        self, chunk: pd.DataFrame, target_vector: np.ndarray, method: str
    ) -> List[Dict[str, Any]]:
    ...
    def analyze_similar_matches(
        self, similar_matches: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _get_cached_similarity(
        self, odds_hash: str, method: str, threshold: float
    ...
    def get_method_comparison(
        self, target_odds: Dict[str, float]
