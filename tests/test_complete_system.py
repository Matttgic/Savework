...
class TestSimilarityEngine(unittest.TestCase):
    ...
    def test_similarity_calculations(self):
        ...
        # Test euclidienne
        similarity_euclidean = self.engine.calculate_similarity_euclidean(
            vector1, vector2
        )
        self.assertGreater(similarity_euclidean, 0.9)

        # Test pourcentage
        similarity_percentage = self.engine.calculate_similarity_percentage(
            vector1, vector2
        )
    ...
    def test_find_similar_matches(self):
        ...
        similar_matches = self.engine.find_similar_matches(
            target_odds, method="cosine", threshold=0.8, min_matches=1
        )
...
class TestMetricsCollector(unittest.TestCase):
    ...
    def test_increment_counter(self):
        ...
        counter_keys = [
            k for k in app_metrics["counters"].keys() if "test.counter" in k
        ]
    ...
    def test_timer_functionality(self):
        ...
        app_metrics = self.metrics.get_application_metrics()
        histogram_keys = [
            k for k in app_metrics["histograms"].keys() if "duration" in k
        ]
...
class TestDataCollector(unittest.TestCase):
    ...
    def test_get_sports(self, mock_get):
        ...
        mock_response.json.return_value = {"sports": [{"id": 1, "name": "Football"}]}
    ...
    def test_extract_odds_from_event(self):
        ...
        test_event = {
            ...
            "periods": {
                "num_0": {
                    "money_line": {"home": 2.10, "draw": 3.40, "away": 3.20},
    ...
    def test_league_filtering_case_insensitive(self):
        """Les ligues autorisées doivent être comparées sans tenir compte de la casse"""
        event = {"league_name": "PREMIER LEAGUE", "event_id": 1}
        markets_data = {"events": [event]}

        with patch.object(
            self.collector, "get_markets", return_value=markets_data
        ), patch.object(
            self.collector, "extract_odds_from_event", return_value=event
        ), patch.object(
            self.collector, "_has_complete_odds", return_value=True
        ), patch(
            "src.data_collector.Config.ALLOWED_LEAGUES", ["premier league"]
        ), patch.object(
            self.collector.db_manager, "save_match"
        ) as mock_save:
            result = self.collector.collect_current_markets()
            self.assertEqual(result["collected"], 1)
            mock_save.assert_called_once()

        # Non autorisé
        markets_data = {"events": [{"league_name": "Serie A", "event_id": 2}]}
        with patch.object(
            self.collector, "get_markets", return_value=markets_data
        ), patch.object(
            self.collector, "extract_odds_from_event", return_value=event
...
def run_performance_tests():
    ...
    try:
        ...
        print(
            f"Temps d'insertion: {insertion_time:.2f}s ({1000/insertion_time:.1f} matchs/s)"
        )
        ...
        similar_matches = engine.find_similar_matches(
            target_odds, method="cosine", threshold=0.8, min_matches=10
        )
        ...
        print(
            f"Statistiques finales: {stats['total_matches']} matchs, {stats['matches_with_odds']} avec cotes complètes"
        )
